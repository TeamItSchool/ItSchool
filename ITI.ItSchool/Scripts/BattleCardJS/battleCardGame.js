function loadBattleCardGame(choice) {
    ...
    var scoreCounter = 0;
    var values = [];
    var size = false; // check if we have to change the text size in cards
    if (choice == "0-10") {
        for (var i = 0; i != 11; ++i) values.push(i)
    }
    else if (choice == "alphabet") {
        for (var i = 0; i != 26; ++i) values.push(String.fromCharCode(i + 65));
    }
    else if (choice == "0-100") {
        size = true;
        for (var i = 0; i != 101; ++i) values.push(i)
    }
    else if (choice == "0-500") {
        size = true;
        for (var i = 0; i != 501; ++i) values.push(i)
    }
